...
WiFiClient client;
WiFiClient dclient;
bool debug = 0, delete_file = 1;
long count1 = 0, count2 = 0, count3 = 0, count3_delay = 100;
...
void setup() {
  ...
  write_status(CMD18);
  while ((WiFi.status() != WL_CONNECTED)&&waiting_wifi>=1)
  {
    waiting_wifi--;
    delay(500);
...
void loop()
{
  ...
  if ((millis() - count3) > count3_delay)
  {
    ...
    {
      mydata.command = CMD6;// read input
      ET.sendData();
    }
    count3 = millis();
  }
